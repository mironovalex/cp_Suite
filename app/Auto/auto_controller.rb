...
def tst
  ...
  a = a.force_encoding("UTF-8")

    
  #WebView.execute_js("msg('" + a.encoding.name + "');")
...
def sync_one
  ...
  $msg = ""
  $sync_msg = ""
  $sync_msg = "Синхронизация: Системные операции" 
...
#         )      
 ...
 def inp_location_
  ...
  def inp_location                 
    ...
    count = TsdInventlocation.find(:count)        
...
#               )     
    ...
    TsdLog.create("UserId" => "", 
                  ...
                  "Operation" => "3",
                  "Status" => status,                  
                  "Comment" => "TsdInventlocation: " + count.to_s + " строк"
                  )      
 ...
 def inp_location_3  
   ...
   TsdLog.create("UserId" => "", 
                 ...
                 "Operation" => "2",
                 "Status" => status,                  
                 "Comment" => "TsdUserinfo: " + count.to_s + " строк"
                 )            
  ...
  def user_check
    ...
    if (@@USER2 == "")
      ...
      if (!@TsdUserinfos)            
        ...
        $chk_userid =  @TsdUserinfos.UserId
        if (@TsdUserinfos.SetPassword == "1" or @TsdUserinfos.Password == "")
          #WebView.navigate(url_for(:action => :inp_password_conf)) 
          $msg = ""
...
#      @TsdUserinfoss = TsdUserinfo.find(:all,
#               :conditions => { 
#               {
#               :name => "UserId", 
#               :op => "IN" 
...
#               } )  
      ...
      @TsdUserinfoss = TsdUserinfo.find(:all)                
      @TsdUserinfoss.each do |e|     
        if (e.UserId.upcase == @@USER2.upcase)
           tmp = 1; 
           $chk_user = e.UserName;
           $chk_userid = @@USER2;  
      
                          @TsdUserinfosss = TsdUserinfo.find(:first,
                                   :conditions => { 
                                   {
                                   :name => "UserId", 
                                   :op => "IN" 
        ...
        if (tmp == 1)  
          ...
          WebView.execute_js("alarm();") 
          $msg = "Не сушествующий пользователь!"
          redirect :action => :inp_login               
        end                       
    end        
...
#    @TsdUserinfoss = TsdUserinfo.find(:first,
#             :conditions => { 
#             {
#             :name => "UserId", 
#             :op => "IN" 
...
#    
#    if (@TsdUserinfoss)   
#      $chk_user = @TsdUserinfoss.UserName
#      $chk_userid = @@USER2
#    else    
#      $chk_user = @@USER1         
#      @TsdUserinfos = TsdUserinfo.find(:first,
...
#    end    
  ...
  def loc_check   
    ...
    if (@TSDInventlocations)
      @@LOC = @@LOCID
      $chk_loc_name = @TSDInventlocations.InventLocationName      
    end  

    @TSDInventlocationss = TsdInventlocation.find(:first,
          :conditions => { 
          {
            :name => "InventLocationId", 
            :op => "IN" 
    ...
    $chk_loc_name = @TSDInventlocationss.InventLoationName          
       
#    Alert.show_popup(
#        :message=> @@LOC.to_s + "|2|" + @@LOCID.to_s + $chk_loc_name.to_s ,
#        :title=>"Сообщение",
#        :buttons => ["Ok"]
#     )  
       
#    Alert.show_popup(
#        :message=>@TSDInventlocations.InventLocationName,
#        :title=>"Сообщение",
#        :buttons => ["Ok"]
#     )  
